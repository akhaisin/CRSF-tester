...
- Renamed project to `crsf-tester` in `package.json`.
...
- Updated `index.html` title to `CRSF Tester`.
- Added dependencies:
...
- Set `base: '/CRSF-tester/'` in `vite.config.ts` so asset paths resolve correctly under the Pages sub-path.
